...
class XenditController extends Controller
{
  public function purchaseProcess(Request $request)
  {
    ...
    if (!empty($response['success_redirect_url'])) {
      ...
      return redirect()->route('shop.checkout')->with(['alert-type' => 'error', 'message' => $response['message']]);
    }
  }
  ...
  public function notify(Request $request)
  {
    ...
    if ($response->failed()) {
      return redirect()->route('shop.checkout')->with(['alert-type' => 'error', 'message' => 'Failed to verify payment.']);
    }
