                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6 offset-lg-3">
                            <form id="ajaxForm" action="{{ route('artist.withdraw.store') }}" method="POST"
                                ...
                                @if (isset($errors) && (is_array($errors) ? count($errors) > 0 : $errors->any()))
                                    <div class="alert alert-danger">
                                        ...
                                        <ul>
                                            @foreach ($errors instanceof \Illuminate\Support\ViewErrorBag ? $errors->all() : $errors as $error)
                                                <li>{{ $error }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="withdraw_method_id" id="withdraw_method" class="form-control" required>
                                        <option selected disabled value="">{{ __('Select Withdraw Method') }}</option>
                                        @foreach ($methods as $item)
                                            <option value="{{ $item->id }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                    <p id="err_withdraw_method_id" class="mt-2 mb-0 text-danger em"></p>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    @if (isset($errors) && ($errors instanceof \Illuminate\Support\ViewErrorBag ? $errors->has('additional_reference') : isset($errors['additional_reference'])))
                                        <p class="mt-2 mb-0 text-danger">{{ $errors instanceof \Illuminate\Support\ViewErrorBag ? $errors->first('additional_reference') : $errors['additional_reference'] }}</p>
                                    @endif
                                </div>
                            </form>
                        </div>
...
@endsection
@section('script')
    <script src="{{ asset('assets/admin/js/artist-withdraw.js') }}"></script>
@endsection