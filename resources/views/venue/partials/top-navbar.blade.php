<div class="main-header">
  ...
  <nav class="navbar navbar-header navbar-expand-lg"
    data-background-color="{{ Auth::guard('venue')->user()->theme_version == 'light' ? 'white' : 'dark' }}">
    <div class="container-fluid">
      <ul class="navbar-nav topbar-nav ml-md-auto align-items-center">
        <form action="{{ route('venue.change_theme') }}" class="form-inline mr-3" method="POST">
            ...
            <div class="selectgroup selectgroup-secondary selectgroup-pills">
              <label class="selectgroup-item">
                <input type="radio" name="theme_version" value="light" class="selectgroup-input"
                  {{ Auth::guard('venue')->user()->theme_version == 'light' ? 'checked' : '' }}
              ...
              <label class="selectgroup-item">
                <input type="radio" name="theme_version" value="dark" class="selectgroup-input"
                  {{ Auth::guard('venue')->user()->theme_version == 'dark' ? 'checked' : '' }}
                  onchange="this.form.submit()">
          ...
          <ul class="dropdown-menu dropdown-user animated fadeIn">
            <div class="dropdown-user-scroll scrollbar-outer">
              <li>
                <div class="user-box">
                  <div class="avatar-lg">
                    @if (Auth::guard('venue')->user()->photo != null)
                      <img
                        src="{{ asset('assets/admin/img/venue-photo/' . Auth::guard('venue')->user()->photo) }}"
                        alt="Admin Image" class="avatar-img rounded-circle">
                    @else
                      <img src="{{ asset('assets/admin/img/blank_user.jpg') }}" alt=""
                        class="avatar-img rounded-circle">
                  ...
                  <div class="u-text">
                    <h4>
                      {{ Auth::guard('venue')->user()->first_name . ' ' . Auth::guard('venue')->user()->name }}
                    </h4>
                    <p class="text-muted">{{ Auth::guard('venue')->user()->email }}</p>
                  </div>
                </div>
              </li>
              ...
              <li>
                ...
                <a class="dropdown-item" href="{{ route('venue.edit.profile') }}">
                  {{ __('Edit Profile') }}
                </a>

                <div class="dropdown-divider"></div>
                <a class="dropdown-item" href="{{ route('venue.change.password') }}">
                  {{ __('Change Password') }}
                </a>
