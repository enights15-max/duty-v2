<script>
    ...
    const baseUrl = "{{ url('/') }}";
    const countryUrl = "{{ route('venue.get_country') }}";
    const stateUrl = "{{ route('venue.get_state') }}";
    const cityUrl = "{{ route('venue.get_city') }}";
</script>
...
<script>
    'use strict';
    const account_status = {{ Auth::guard('venue')->user()->status }};
