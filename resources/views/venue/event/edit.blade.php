...
@section('content')
    <div class="page-header">
        ...
        <ul class="breadcrumbs">
            ...
            <li class="nav-item">
                <a
                    href="{{ route('venue.event_management.event', ['language' => $defaultLang->code]) }}">{{ __('All Events') }}</a>
            </li>
            <li class="separator">
                <i class="flaticon-right-arrow"></i>
            </li>
                    ...
                    <div class="row">
                        <div class="col-lg-8 offset-lg-2">
                            ...
                            <div class="col-lg-12">
                                ...
                                <form action="{{ route('venue.event.imagesstore') }}" id="my-dropzone"
                            ...
                            <form id="eventForm" action="{{ route('venue.event.update') }}" method="POST"
...
@section('variables')
    ...
    <script>
        ...
        var storeUrl = "{{ route('venue.event.imagesstore') }}";
        var removeUrl = "{{ route('venue.event.imagermv') }}";

        var rmvdbUrl = "{{ route('venue.event.imgdbrmv') }}";
        var loadImgs = "{{ route('venue.event.images', $event->id) }}";
        var defaultLang = "{{ $defaultLang->code }}";

        const haveCoSt = {{ $haveCoSt }};
        const isActiveState = {{ $settings->event_state_status == 1 ? 1 : 0 }};
        const getStateUrl = "{{ route('venue.get.city.state') }}";
        const getCityUrl = "{{ route('venue.get.cities.state') }}";
    </script>
    @if ($settings->google_map_status == 1)
        <script>
