...
@section('content')
  ...
  <div class="page-header">
    ...
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a href="{{ route('venue.event_reservation.index', ['status' => 'all']) }}">{{ __('Reservations') }}</a>
      </li>
      <li class="separator">
        <i class="flaticon-right-arrow"></i>
      </li>
  ...
  <div class="row">
    <div class="col-lg-8">
      @include('backend.event.reservation.partials.details-core', [
        'backUrl' => route('venue.event_reservation.index', ['status' => 'all']),
    ...
    <div class="col-lg-4">
      <div class="card">
        <div class="card-header">
          <div class="card-title">{{ __('Venue Actions') }}</div>
        </div>
        <div class="card-body">
          <div class="alert alert-warning">
            {{ __('Venues can operate reservation timelines and inventory for their own events. Refunds remain admin-managed.') }}
          </div>

          @if ($actions['can_extend'])
            <form action="{{ route('venue.event_reservation.extend', ['id' => $reservation->id]) }}" method="POST" class="mb-4">
              @csrf
          ...
          @if ($actions['can_reactivate'])
            <form action="{{ route('venue.event_reservation.reactivate', ['id' => $reservation->id]) }}" method="POST" class="mb-4">
              @csrf
          ...
          @if ($actions['can_convert'])
            <form action="{{ route('venue.event_reservation.convert', ['id' => $reservation->id]) }}" method="POST" class="mb-4"
          ...
          @if ($actions['can_default'])
            <form action="{{ route('venue.event_reservation.default', ['id' => $reservation->id]) }}" method="POST" class="mb-3"
          ...
          @if ($actions['can_cancel'])
            <form action="{{ route('venue.event_reservation.cancel', ['id' => $reservation->id]) }}" method="POST"
          ...
          @if (!$actions['can_extend'] && !$actions['can_reactivate'] && !$actions['can_convert'] && !$actions['can_default'] && !$actions['can_cancel'])
            <p class="text-muted mb-0">{{ __('No venue actions are available for the current reservation state.') }}</p>
          @endif
        </div>
      </div>
    </div>
