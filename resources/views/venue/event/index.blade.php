  ...
  <div class="page-header">
    ...
    <ul class="breadcrumbs">
      ...
      @if (request()->filled('event_type') && request()->input('event_type') == 'venue')
        ...
        <li class="nav-item">
          <a
            href="{{ route('venue.event_management.event', ['language' => $defaultLang->code, 'event_type' => 'venue']) }}">{{ __('Venue Events') }}</a>
        </li>
      @endif
                        ...
                        <tr>
                          ...
                          <td>
                            @if ($event->event_type == 'venue')
                              <a href="{{ route('venue.event.ticket', ['language' => request()->input('language'), 'event_id' => $event->id, 'event_type' => $event->event_type]) }}"
                                class="btn btn-success btn-sm">{{ __('Manage') }}</a>
                            @endif
                          </td>
                          <td>
                            <form id="statusForm-{{ $event->id }}" class="d-inline-block"
                              action="{{ route('venue.event_management.event.event_status', ['id' => $event->id, 'language' => request()->input('language')]) }}"
                              method="post">
                          ...
                          <td>
                            ...
                            <form id="featuredForm-{{ $event->id }}" class="d-inline-block"
                              action="{{ route('venue.event_management.event.update_featured', ['id' => $event->id]) }}"
                              method="post">
                          ...
                          <td>
                            <div class="dropdown">
                              ...
                              <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a href="{{ route('venue.event_management.edit_event', ['id' => $event->id]) }}"
                                  class="dropdown-item">
                                  {{ __('Edit') }}
                                </a>

                                <a href="{{ route('venue.event_management.ticket_setting', ['id' => $event->id]) }}"
                                  class="dropdown-item">
                                  {{ __('Ticket Settings') }}
                                </a>

                                <form class="deleteForm d-block"
                                  action="{{ route('venue.event_management.delete_event', ['id' => $event->id]) }}"
                                  method="post">
