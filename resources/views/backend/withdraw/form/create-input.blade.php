<div class="card ops-panel">
  ...
  <form id="ajaxForm" action="{{ route('admin.withdraw_payment_method.store_input') }}" method="post"
    ...
    <div class="card-body">
    <span class="scarlet-modal__eyebrow">{{ __('Field blueprint') }}</span>
    <h6 class="scarlet-modal__title">{{ __('Add a new payout field') }}</h6>
    <p class="scarlet-modal__intro">
      {{ __('Use only the information finance or ops actually needs to complete the withdrawal safely.') }}
    </p>
    ...
    <div class="scarlet-modal__section">
      <div class="form-group">
        ...
        <small class="scarlet-form-hint">{{ __('Prefer text, number or select inputs for finance operations. Use checkbox and textarea only when strictly necessary.') }}</small>
      </div>
