...
$(function ($) {
  ...
  // Uploaded Image Preview 299 Start
  $('.img-input299').on('change', function (event) {
    let file = event.target.files[0];
    let reader = new FileReader();

    reader.onload = function (e) {
      $('.uploaded-img299').attr('src', e.target.result);
    };
    reader.readAsDataURL(file);
  });
  // Uploaded Image Preview 299 End
  ...
  // Uploaded Image Preview 4 Start
  ...
  $('select[name="language_id"]').on('change', function () {
    ...
    let get_country = typeof getCountry !== 'undefined' ? getCountry : null;
    let get_state = typeof getState !== 'undefined' ? getState : null;

    let rtlURL = baseUrl + "/admin/language-management/" + $(this).val() + "/check-rtl";

    let params = [];
    if (get_country !== null) params.push("country=" + encodeURIComponent(get_country));
    if (get_state !== null) params.push("state=" + encodeURIComponent(get_state));
    if (params.length > 0) rtlURL += "?" + params.join("&");
    ...
    $.get(rtlURL, function (response) {
      ...
      if (!('successData' in response)) {
        alert(response.errorData || 'Unknown error');
        return;
      }

      if ('successData' in response) {
        // initialize country select with AJAX (pass lang from data-lang attr)
        ...
        //get states of selected language
