...
trait ApiFormatTrait
{
  ...
  protected function format_organizer_data($data, $type)
  {
    ...
    $obj->phone = $type == 'admin' ? ($data->phone ?? null) : ($data->phone ?? null);
    $obj->email = $type == 'admin' ? ($data->email ?? null) : ($data->email ?? null);
    $obj->username = $type == 'admin' ? ($data->username ?? null) : ($data->username ?? null);
    $obj->status = $type == 'admin' ? 1 : ($data->status ?? 0);
    $obj->facebook = $type == 'admin' ? null : ($data->facebook ?? null);
    $obj->twitter = $type == 'admin' ? null : ($data->twitter ?? null);
    $obj->linkedin = $type == 'admin' ? null : ($data->linkedin ?? null);
    $obj->organizer_name = $type == 'admin' ? (($data->first_name ?? "") . ' ' . ($data->last_name ?? "")) : ($data->organizer_name ?? "");
    $obj->country = $type == 'admin' ? null : ($data->country ?? null);
    $obj->city = $type == 'admin' ? null : ($data->city ?? null);
    $obj->state = $type == 'admin' ? null : ($data->state ?? null);
    $obj->address = $type == 'admin' ? ($data->address ?? null) : ($data->address ?? null);
    $obj->zip_code = $type == 'admin' ? null : ($data->zip_code ?? null);
    $obj->designation = $type == 'admin' ? null : ($data->designation ?? null);
    $obj->details = $type == 'admin' ? ($data->details ?? null) : ($data->details ?? null);
    $obj->user_type = $type;
    return $obj;
  }
  ...
  protected function format_sender_data($data, $type)
  {
    ...
    $obj->phone = $type == 'admin' ? ($data->phone ?? null) : ($data->phone ?? null);
    $obj->address = $type == 'admin' ? null : ($data->address ?? null);
    $obj->country = $type == 'admin' ? null : ($data->country ?? null);
    $obj->city = $type == 'admin' ? null : ($data->city ?? null);
    $obj->state = $type == 'admin' ? null : ($data->state ?? null);
    $obj->zip_code = $type == 'admin' ? null : ($data->zip_code ?? null);
