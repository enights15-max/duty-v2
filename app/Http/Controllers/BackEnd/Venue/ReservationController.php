...
class ReservationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $venueId = (int) Auth::guard('venue')->id();
        $eventIds = $this->resolveEventIdsByTitle($eventTitle);

        $reservations = $this->buildScopedQuery($venueId, $status, $queryText, $eventIds, $refundState, $dueState)
        ...
        $metrics = $this->buildMetrics($venueId);

        return view('venue.event.reservation.index', compact(
            'defaultLang',
            'reservations',
            'status',
            'queryText',
    ...
    public function export(Request $request)
    {
        ...
        $reservations = $this->decorateReservationCollection(
            $this->buildScopedQuery($venueId, $status, $queryText, $eventIds, $refundState, $dueState)
                ->with($this->reservationListRelations())
                ->orderByRaw($this->statusOrderSql())
                ->orderByRaw("CASE WHEN status = 'active' THEN COALESCE(expires_at, final_due_date, created_at) END ASC")
                ->orderByDesc('id')
        ...
        $filename = 'venue-reservations-' . now()->format('Ymd_His') . '.csv';
    ...
    public function show($id)
    {
        ...
        return view('venue.event.reservation.details', compact(
            'defaultLang',
            'reservation',
            'paymentSummary',
            'refundSummary',
    ...
    private function buildScopedQuery(int $venueId, string $status, string $queryText, array $eventIds, string $refundState, string $dueState): Builder
    {
        $query = TicketReservation::query()
            ->whereHas('event', function (Builder $query) use ($venueId) {
    ...
    private function buildMetrics(int $venueId): array
    {
        $query = TicketReservation::query()
            ->whereHas('event', function (Builder $eventQuery) use ($venueId) {
                $eventQuery->where('venue_id', $venueId);
            });
    ...
    private function reservationListRelations(): array
    {
        return [
            ...
            'event:id,venue_id,venue_identity_id,thumbnail,end_date_time',
            ...
            'bookings:id,reservation_id,booking_id,order_number,paymentStatus,event_id',
        ];
    }
